...
from src.models.components.net_swin import Dual_MTLR as Swin
from src.models.components.net_cnn import Dual_MTLR as CNN
...
class DEEP_MTLR(LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.hparams['model'] == 'ViT':
            ...
            self.model = CNN(hparams = self.hparams)
        else:
    ...
    def step(self, batch: Any):
        (sample, clin_var), (y1,y2,y3,y4,y5), labels = batch
        logits1, logits2,logits3, logits4, logits5 = self.forward((sample,clin_var))
        ...
        loss_mtlr += mtlr_neg_log_likelihood(logits5, y5.float(), self.model, self.hparams['C1'], average=True)
        # print('loss_mtlr', loss_mtlr)

        loss = loss_mtlr
        

        return loss, logits1, logits2,logits3, logits4, logits5, labels

    def training_step(self, batch: Any, batch_idx: int):
        loss, preds1, preds2, preds3, preds4, preds5, labels = self.step(batch)

        self.log("train/loss", loss, on_step=False, on_epoch=True, prog_bar=False)

        return {"loss": loss, "preds1": preds1, "preds2": preds2, "preds3": preds3, "preds4": preds4, "preds5": preds5,"labels": labels}
    ...
    def training_epoch_end(self, outputs: List[Any]):
        ...
        pred_prob5   = torch.cat([x["preds5"] for x in outputs]).cpu() 
        true_time5   = torch.cat([x["labels"]["time5"] for x in outputs]).cpu()
        true_event5  = torch.cat([x["labels"]["event5"] for x in outputs]).cpu()
        pred_risk5 = mtlr_risk(pred_prob5).detach().numpy()  
        ci_event5  = concordance_index(true_time5, -pred_risk5, event_observed=true_event5)
    ...
    def validation_epoch_end(self, outputs: List[Any]):
        ...
        print('val SPFS ci_event is ',ci_event5)
        ...
        results = pd.DataFrame({'ID':PatientID, 'OS_risk':pred_risk1, 'OS':true_time1,'Death':true_event1,\
                                ...
                                'SPFS_risk':pred_risk5, 'SPFS':true_time5,'SP':true_event5})
        results.to_csv('Predictions.csv')

        return {"loss": loss, "OS-CI": ci_event1, "LFFS-CI": ci_event2, "RFFS-CI": ci_event3, "DFFS-CI": ci_event4, "SPFS-CI": ci_event5}
