...
class HECKTORDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        df = pd.read_csv(self.hparams["data_dir"]+'/Clinical_List_5_Outcome.csv')
        ...
        key = 1
        for i,j in kf.split(df,df['RF']): # split the dataset based on the event/no-event patient distribution of one outcome (random selection but please not os)
            train_idx[key] = i
            test_idx[key] = j
